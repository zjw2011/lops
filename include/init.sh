...
CentOS_Dependent()
{
    if [ -s /etc/yum.conf ]; then
        \cp /etc/yum.conf /etc/yum.conf.lops
    ...
    if [ -s /etc/yum.conf.lops ]; then
        mv -f /etc/yum.conf.lops /etc/yum.conf
    fi
}
...
CentOS_Lib_Opt()
{
    if [ `grep -L 'fs.file-max=65535'    '/etc/sysctl.conf'` ]; then
